...
try {
  enabledCategories = JSON.parse(
    localStorage.getItem("rdr2collector.enabled-categories") ||
      localStorage.getItem("enabled-categories"),
  ) || [...categories];
} catch (error) {
...
L.DivIcon.DataMarkup = L.DivIcon.extend({
  _setIconStyles: function (img, name) {
    ...
    if (this.options.tippy) img.dataset.tippy = this.options.tippy;
  },
});
...
function init() {
  try {
    Sentry.init({
      release: nocache,
      tracesSampleRate: isLocalHost() ? 1 : 0.3,
    });
  } catch (err) {
  ...
  Settings.language = Language.availableLanguages.includes(Settings.language)
    ? Settings.language
    : "en";
  ...
  Object.keys(localStorage).forEach((key) => {
    if (key.startsWith("main.")) {
      localStorage.setItem(
        `rdr2collector.${key.replace("main.", "")}`,
  ...
  const itemsCollectionsWeekly = Promise.all([mapping, jewelryTimestamps]).then(
    () => Item.init(),
  ...
  const markers = Promise.all([itemsCollectionsWeekly, lootTables]).then(
    Marker.init,
  );
  const cycles = Promise.all([
    itemsCollectionsWeekly,
    markers,
    setMapTime,
  ]).then(Cycles.load);
  ...
  if (!MapBase.isPreviewMode) MadamNazar.loadMadamNazar();
  ...
  document.querySelectorAll(".map-alert").forEach((alert) => {
    alert.style.display = Settings.alertClosed ? "none" : "";
  });

  document.getElementById("language").value = Settings.language;
  document.getElementById("marker-opacity").value = Settings.markerOpacity;
  document.getElementById("invisible-removed-markers").checked =
    Settings.isInvisibleRemovedMarkers;
  document.getElementById("override-search").checked =
    Settings.overrideBrowserSearch;
  ...
  document.getElementById("marker-cluster").checked =
    Settings.isMarkerClusterEnabled;
  document.getElementById("tooltip-map").checked = Settings.showTooltipsMap;
  document.getElementById("enable-marker-popups").checked =
    Settings.isPopupsEnabled;
  document.getElementById("enable-marker-popups-hover").checked =
    Settings.isPopupsHoverEnabled;
  document.getElementById("enable-marker-shadows").checked =
    Settings.isShadowsEnabled;
  document.getElementById("enable-legendary-backgrounds").checked =
    Settings.isLaBgEnabled;
  document.getElementById("legendary-animal-marker-type").value =
    Settings.legendarySpawnIconType;
  document.getElementById("legendary-animal-marker-size").value =
    Settings.legendarySpawnIconSize;
  document.getElementById("enable-dclick-zoom").checked =
    Settings.isDoubleClickZoomEnabled;
  document.getElementById("pins-place-mode").checked =
    Settings.isPinsPlacingEnabled;
  document.getElementById("pins-edit-mode").checked =
    Settings.isPinsEditingEnabled;
  document.getElementById("show-help").checked = Settings.showHelp;
  document.getElementById("show-coordinates").checked =
    Settings.isCoordsOnClickEnabled;
  document.getElementById("map-boundaries").checked =
    Settings.isMapBoundariesEnabled;
  ...
  document.getElementById("enable-cycle-input").checked =
    Settings.isCycleInputEnabled;
  document.getElementById("enable-right-click").checked =
    Settings.isRightClickEnabled;
  document.getElementById("enable-debug").checked = Settings.isDebugEnabled;
  document.getElementById("enable-cycle-changer").checked =
    Settings.isCycleChangerEnabled;

  document.getElementById("show-utilities").checked =
    Settings.showUtilitiesSettings;
  document.getElementById("show-customization").checked =
    Settings.showCustomizationSettings;
  document.getElementById("show-routes").checked = Settings.showRoutesSettings;
  document.getElementById("show-import-export").checked =
    Settings.showImportExportSettings;
  document.getElementById("show-debug").checked = Settings.showDebugSettings;

  document.getElementById("help-container").style.display = Settings.showHelp
    ? ""
    : "none";

  document.querySelectorAll(".input-cycle").forEach((el) => {
    el.classList.toggle("hidden", !Settings.isCycleInputEnabled);
  });
  document.querySelectorAll(".cycle-icon").forEach((el) => {
    el.classList.toggle(
      "hidden",
      !Settings.isCyclesVisible || Settings.isCycleInputEnabled,
    );
  });
  document.querySelectorAll(".cycle-display").forEach((el) => {
    el.classList.toggle("hidden", !Settings.isCyclesVisible);
  });
  document
    .getElementById("cycle-changer-container")
    .classList.toggle("hidden", !Settings.isCycleChangerEnabled);

  document
    .getElementById("utilities-container")
    .classList.toggle("opened", Settings.showUtilitiesSettings);
  document
    .getElementById("customization-container")
    .classList.toggle("opened", Settings.showCustomizationSettings);
  document
    .getElementById("routes-container")
    .classList.toggle("opened", Settings.showRoutesSettings);
  document
    .getElementById("import-export-container")
    .classList.toggle("opened", Settings.showImportExportSettings);
  document
    .getElementById("debug-container")
    .classList.toggle("opened", Settings.showDebugSettings);

  if (!MapBase.isPreviewMode) Updates.init();
...
function changeCursor() {
  const isCoordsEnabled =
    Settings.isCoordsOnClickEnabled || RouteSettings.customRouteEnabled;
  const pointer = "url(assets/images/crosshair_thick.png) 12 12, pointer";
  document.querySelector(".leaflet-grab").style.cursor = isCoordsEnabled
    ? pointer
    : "grab";
...
function clockTick() {
  ...
  if (timeInGame)
    timeInGame.textContent = gameTime.toLocaleString(
      Settings.language,
      clockFormat,
    );
  ...
  if (dayCycleEl) {
    ...
    if ((nightTime && file !== "moon") || (!nightTime && file !== "sun")) {
      ...
      dayCycleEl.setAttribute(
        "src",
        `./assets/images/${nightTime ? "moon" : "sun"}.png`,
      );
    }
  }

  const cycleResetTime = Date.UTC(
    now.getUTCFullYear(),
    now.getUTCMonth(),
    now.getUTCDate() + 1,
  );
  ...
  if (countdown) countdown.textContent = delta.toLocaleString([], deltaFormat);

  document
    .querySelectorAll(
      '[data-marker*="provision_wldflwr_agarita"], [data-marker*="provision_wldflwr_blood_flower"]',
    )
    .forEach((marker) => {
      const isImportant = marker.classList.contains("highlight-items");
      const whiteGlow =
        "drop-shadow(0 0 .5rem #fff) drop-shadow(0 0 .3rem #fff)";
      const redGlow =
        "drop-shadow(0 0 .5rem #cc0000) drop-shadow(0 0 .4rem #cc0000)";
      const pinkGlow =
        "drop-shadow(0 0 .5rem #ff6fc7) drop-shadow(0 0 .3rem #ff6fc7)";
  ...
  document
    .querySelectorAll(".leaflet-marker-icon[data-time]")
    .forEach((marker) => {
      let time = marker.dataset.time || "";
      if (time === "") return;
      if (time.split(",").includes(gameHour + "") && !MapBase.isPreviewMode) {
        marker.style.filter =
          "drop-shadow(0 0 .5rem #fff) drop-shadow(0 0 .25rem #fff)";
      } else {
        marker.style.filter = "none";
      }
    });
...
const draggableBackToTop = draggify(backToTop, {
  storageKey: "rdr2collector.backToTopPosition",
});
let wasAtTop = true;
...
sideMenu.addEventListener("scroll", function () {
  ...
  const atBottom =
    Math.abs(this.scrollHeight - this.scrollTop - this.clientHeight) < 1;
...
document
  .getElementById("enable-right-click")
  .addEventListener("change", function () {
...
document
  .getElementById("show-customization")
  .addEventListener("change", function () {
...
document
  .getElementById("show-import-export")
  .addEventListener("change", function () {
...
document
  .getElementById("enable-cycle-changer")
  .addEventListener("change", function () {
...
searchInput.addEventListener("input", function () {
  ...
  if (!isMobile()) {
    suggestionsHotkeys.style.display =
      searchInput.value.trim() === "" ? "" : "none";
    document
      .querySelectorAll("#weekly-container .header, #weekly-container p")
...
searchInput.addEventListener("focus", function () {
  if (!isMobile()) {
    suggestionsHotkeys.classList.toggle(
      "focused",
...
searchInput.addEventListener("keydown", function (e) {
  const suggestions =
    suggestionsContainer.querySelectorAll(".query-suggestion");
  switch (e.key) {
    case "ArrowDown":
      e.preventDefault();
...
document
  .getElementById("copy-search-link")
...
document
  .getElementById("disable-all-collected-items")
  .addEventListener("click", function () {
...
document
  .getElementById("clear-custom-routes")
  .addEventListener("click", Routes.clearCustomRoutes);
...
document
  .getElementById("show-coordinates")
  .addEventListener("change", function () {
...
document
  .getElementById("invisible-removed-markers")
  .addEventListener("change", function () {
...
document
  .getElementById("filter-min-amount-items")
  .addEventListener("change", function () {
    InventorySettings.maxAmountLowInventoryItems = Number(this.value);
  });

document
  .getElementById("enable-cycles")
  .addEventListener("change", function () {
...
document
  .getElementById("enable-cycle-input")
  .addEventListener("change", function () {
...
document.addEventListener("click", function ({ target }) {
  if (target.closest(".collectible-wrapper[data-type]")) {
    const collectible = target
      .closest(".collectible-wrapper")
      .getAttribute("data-type");
    const category = target
      .closest(".collectible-wrapper")
      .parentElement.getAttribute("data-type");

    MapBase.removeItemFromMap(
      Cycles.categories[category],
      collectible,
      collectible,
      category,
      !InventorySettings.isMenuUpdateEnabled,
    );
  }
});
...
document.querySelector(".menu-toggle").addEventListener("click", function () {
  ...
  document
    .querySelector(".top-widget")
    .classList.toggle("top-widget-menu-opened", isMenuOpened);
  document
    .getElementById("fme-container")
    .classList.toggle("fme-menu-opened", isMenuOpened);
});
...
document
  .getElementById("enable-marker-popups")
  .addEventListener("change", function () {
...
document
  .getElementById("enable-marker-popups-hover")
  .addEventListener("change", function () {
    Settings.isPopupsHoverEnabled = this.checked;
  });

document
  .getElementById("enable-marker-shadows")
  .addEventListener("change", function () {
...
document
  .getElementById("enable-legendary-backgrounds")
  .addEventListener("change", function () {
...
document
  .getElementById("legendary-animal-marker-type")
  .addEventListener("change", function () {
...
document
  .getElementById("legendary-animal-marker-size")
  .addEventListener("change", function () {
...
document
  .getElementById("enable-dclick-zoom")
  .addEventListener("change", function () {
...
document
  .getElementById("enable-inventory")
  .addEventListener("change", function () {
    ...
    document
      ...
      .classList.toggle("opened", InventorySettings.isEnabled);
  });

document
  .getElementById("enable-inventory-popups")
  .addEventListener("change", function () {
...
document
  .getElementById("reset-inventory-daily")
  .addEventListener("change", function () {
    InventorySettings.resetInventoryDaily = this.checked;
  });

document
  .getElementById("enable-additional-inventory-options")
  .addEventListener("change", function () {
    InventorySettings.enableAdvancedInventoryOptions = this.checked;
  });

document
  .getElementById("highlight_low_amount_items")
  .addEventListener("change", function () {
...
document
  .getElementById("enable-inventory-menu-update")
  .addEventListener("change", function () {
    InventorySettings.isMenuUpdateEnabled = this.checked;
  });

document
  .getElementById("auto-enable-sold-items")
  .addEventListener("change", function () {
    InventorySettings.autoEnableSoldItems = this.checked;
  });

document
  .getElementById("inventory-stack")
  .addEventListener("change", function () {
...
document
  .getElementById("soft-flowers-inventory-stack")
  .addEventListener("change", function () {
    const min = parseInt(this.min);
    const max = parseInt(this.max);
    let inputValue = parseInt(this.value);
...
document.getElementById("cookie-export").addEventListener("click", function () {
  try {
    ...
    Object.keys(settings).forEach(function (key) {
      if (!key.startsWith("rdr2collector.")) delete settings[key];
    });
    ...
    downloadAsFile(
      `collectible-map-settings-(${exportDate}).json`,
      settingsJson,
    );
  } catch (error) {
    console.error(error);
...
document
  .getElementById("generate-route-generate-on-visit")
  .addEventListener("change", function () {
    RouteSettings.runOnStart = this.checked;
  });

document
  .getElementById("generate-route-ignore-collected")
  .addEventListener("change", function () {
...
document
  .getElementById("generate-route-important-only")
  .addEventListener("change", function () {
...
document
  .getElementById("generate-route-auto-update")
  .addEventListener("change", function () {
    RouteSettings.autoUpdatePath = this.checked;
  });

document
  .getElementById("generate-route-distance")
  .addEventListener("change", function () {
    let inputValue = parseInt(this.value);
...
document
  .getElementById("generate-route-start")
  .addEventListener("change", function () {
    ...
    document.querySelector(".lat-lng-container").style.display = isMapClick
      ? "block"
      : "none";
    changeCursor();
  });
...
document
  .getElementById("generate-route-start-lat")
  .addEventListener("change", function () {
...
document
  .getElementById("generate-route-start-lng")
  .addEventListener("change", function () {
    let inputValue = parseFloat(this.value);
...
document
  .getElementById("generate-route-use-pathfinder")
  .addEventListener("change", function () {
    ...
    if (RouteSettings.usePathfinder) {
      ...
      document.getElementById(
        "generate-route-auto-update",
      ).parentElement.parentElement.style.display = "none";
      document.getElementById(
        "generate-route-fasttravel-weight",
      ).parentElement.style.display = "";
      document.getElementById(
        "generate-route-railroad-weight",
      ).parentElement.style.display = "";
    } else {
      document.getElementById(
        "generate-route-distance",
...
document
  .getElementById("generate-route-fasttravel-weight")
  .addEventListener("change", function () {
...
document
  .getElementById("generate-route-railroad-weight")
  .addEventListener("change", function () {
...
document
  .getElementById("delete-all-settings")
  .addEventListener("click", function () {
    for (const key in localStorage) {
      if (key.startsWith("rdr2collector.")) localStorage.removeItem(key);
    }
...
const clearMarkersModal = new bootstrap.Modal(
  document.getElementById("clear-markers-modal"),
);
document
  .getElementById("open-clear-markers-modal")
  .addEventListener("click", function () {
    clearMarkersModal.show();
  });

const clearImportantItemsModal = new bootstrap.Modal(
  document.getElementById("clear-important-items-modal"),
);
document
  .getElementById("open-clear-important-items-modal")
  .addEventListener("click", function () {
    clearImportantItemsModal.show();
  });

const clearInventoryModal = new bootstrap.Modal(
  document.getElementById("clear-inventory-modal"),
);
document
  .getElementById("open-clear-inventory-modal")
  .addEventListener("click", function () {
    clearInventoryModal.show();
  });

const clearRoutesModal = new bootstrap.Modal(
  document.getElementById("clear-routes-modal"),
);
document
  .getElementById("open-clear-routes-modal")
  .addEventListener("click", function () {
    clearRoutesModal.show();
  });

const deleteAllSettingsModal = new bootstrap.Modal(
  document.getElementById("delete-all-settings-modal"),
);
document
  .getElementById("open-delete-all-settings-modal")
  .addEventListener("click", function () {
    deleteAllSettingsModal.show();
  });

const removeAllPinsModal = new bootstrap.Modal(
  document.getElementById("remove-all-pins-modal"),
);
document
  .getElementById("open-remove-all-pins-modal")
  .addEventListener("click", function () {
    removeAllPinsModal.show();
  });

const keyboardShortcutsModal = new bootstrap.Modal(
  document.getElementById("keyboard-shortcuts-modal"),
);
document
  .getElementById("open-keyboard-shortcuts-modal")
  .addEventListener("click", function () {
...
const importRDOInventoryModal = new bootstrap.Modal(
  document.getElementById("import-rdo-inventory-modal"),
);
document
  .getElementById("open-import-rdo-inventory-modal")
  .addEventListener("click", function () {
...
function formatLootTableLevel(table, rate = 1, level = 0) {
  ...
  if (hasItems) {
    ...
    Object.keys(items).forEach((key) => {
      wrapper.appendChild(
        formatLootTableLevel(key, rate * items[key], level + 1),
      );
    });
...
const markerTRBLModal = new bootstrap.Modal(
  document.getElementById("marker-troubleshooting-modal"),
);
...
videoModalEl.addEventListener("show.bs.modal", function (event) {
  ...
  const parentDiv = button.firstAncestorOrSelf(
...
const customMarkerColorModal = new bootstrap.Modal(
  document.getElementById("custom-marker-color-modal"),
);
document
  .getElementById("open-custom-marker-color-modal")
...
function filterMapMarkers() {
  ...
  if (Settings.filterType === "none") {
    ...
    if (searchSet.size) MapBase.onSearch(searchInputVal, true);
    else uniqueSearchMarkers = MapBase.markers;

    MapBase.addMarkers();
    return;
  } else if (["moonshiner", "naturalist"].includes(Settings.filterType)) {
    const roleItems = [].concat(
      ...Object.values(MapBase.filtersData[Settings.filterType]),
    );
    filterType = (marker) => roleItems.includes(marker.itemId);
  } else if (Settings.filterType === "weekly") {
    const weeklyItems = Weekly.current.collectibleItems.map(
      (item) => item.itemId,
    );
    ...
    filterType = (marker) =>
      ...
      weeklyItems.some(
        (item) => marker.possibleItems && marker.possibleItems.includes(item),
      );
  } else if (Settings.filterType === "important") {
    const importantItems = Item.items
  ...
  // hides only flowers not belongs to any moonshine recipe
    ...
    const roleItems = [].concat(
      ...Object.values(MapBase.filtersData["moonshiner"]),
    );
    filterType = (marker) =>
      roleItems.includes(marker.itemId) || marker.category !== "flower";
  } else if (Settings.filterType === "coinsSpots") {
    filterType = (marker) =>
      ["coin", "random"].includes(marker.category) && marker.tool === 2;
  ...
  MapBase.markers.filter(filterType).forEach((marker) => {
    ...
    if (!enabledCategories.includes(marker.category) && enableMainCategory) {
      ...
      document
        .querySelectorAll(`[data-type="${marker.category}"]`)
        .forEach((el) => {
...
function loadFont(name, urls = {}) {
  ...
  const fontFace = new FontFace(name, sources, {
    style: "normal",
    weight: "400",
  });
...
function isMobile() {
  ...
  const isMobileUAData = navigator.userAgentData
  ...
  return (
    isCoarsePointer || hasTouchPoints || isMobileUAData || isMobileUserAgent
  );
}
...
function detectPlatform() {
  ...
  const platform =
    userAgentData?.platform || userAgent || vendor || window.opera || "";
  const platforms = {
