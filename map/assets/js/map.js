const MapBase = {
  ...
  mapInit: function () {
    ...
    const mapLayers = {
      "map.layers.default": L.tileLayer(
      ...
      "map.layers.detailed": L.tileLayer(
        (isLocalHost()
          ? "assets/maps/"
          : "https://map-tiles.b-cdn.net/assets/rdr3/") +
          "webp/detailed/{z}/{x}_{y}.webp",
        {
          noWrap: true,
          bounds: mapBoundary,
          attribution: "",
      ...
      "map.layers.dark": L.tileLayer(
        (isLocalHost()
          ? "assets/maps/"
          : "https://map-tiles.b-cdn.net/assets/rdr3/") +
          "webp/darkmode/{z}/{x}_{y}.webp",
        {
          noWrap: true,
          bounds: mapBoundary,
          attribution: "",
      ...
      "map.layers.black": L.tileLayer(
        (isLocalHost()
          ? "assets/maps/"
          : "https://map-tiles.b-cdn.net/assets/rdr3/") +
          "webp/black/{z}/{x}_{y}.webp",
        {
          noWrap: true,
          bounds: mapBoundary,
          attribution: "",
    ...
    document
      .querySelectorAll(".leaflet-control-layers-list span span")
      .forEach((node) => {
    ...
    MapBase.map.doubleClickZoom[
      Settings.isDoubleClickZoomEnabled ? "enable" : "disable"
    ]();
  ...
  setMapTime: function () {
    return Loader.promises["timezone"]
      .consumeJson(({ time }) => {
        ...
        if (Math.abs(difference) > 1e4) {
          ...
          console.info(
            `%c[UTC time] Corrected by ${difference}ms`,
      ...
      .catch(() => {
        console.info(
          "%c[UTC time] Unable to load!",
          "color: #FF6969; background: #242424",
        );
  ...
  isSameUtcDay: function (
    timestamp1,
    timestamp2 = MapBase.mapTime().valueOf(),
  ) {
    if (
      !timestamp1 ||
    ...
    const [date1, date2] = [timestamp1, timestamp2].map((time) =>
      new Date(time).toISOUTCDateString(),
    );
    return date1 === date2;
  },
  ...
  isDarkMode: function () {
    return ["map.layers.dark", "map.layers.black"].includes(
      this.themeOverride || Settings.baseLayer,
    );
  },
  ...
  loadJewelryTimestamps: function () {
    return Loader.promises["jewelry_timestamps"]
      .consumeJson((data) => {
        MapBase.jewelryTimestamps = data.items;
        console.info(
          "%c[Jewelry timestamps] Loaded!",
          "color: #bada55; background: #242424",
        );
      })
      .catch(() => {
        console.info(
          "%c[Jewelry timestamps] Unable to load!",
          "color: #FF6969; background: #242424",
        );
        return Promise.resolve();
      });
  ...
  setMapBackground: function () {
    "use strict";
    document.getElementById("map").style.backgroundColor = MapBase.isDarkMode()
  ...
  setFallbackFonts: async function () {
    const fontsData = {
      ja: {
        ...
        contentUrls: {
          woff2: "assets/fonts/fallback/MotoyaExGothic-W4-KP.woff2",
        },
        title: "MotoyaAporo",
    ...
    if (fontsData[Settings.language]) {
      const { content, contentUrls, title, titleUrls } =
        fontsData[Settings.language];
      ...
      rootStyles.setProperty(
        "--content-font",
        `var(--default-content-font), ${content}, serif`,
      );
      rootStyles.setProperty(
        "--title-font",
        `var(--default-title-font), ${title}, serif`,
      );
    }
  },
  ...
  beforeLoad: function () {
    ...
    if (
      themeParam &&
      ["default", "detailed", "dark", "black"].includes(themeParam)
    )
    ...
    if (
      !isNaN(zoomParam) &&
      this.minZoom <= zoomParam &&
      zoomParam <= this.maxZoom
    )
  ...
  afterLoad: function () {
    ...
    if (previewParam) {
      ...
      document
        .querySelector(
          ".leaflet-zoomex.leaflet-zoomex-rightbottom.leaflet-control",
        )
        .remove();
      ...
      if (isValidCategory) {
        ...
        if (
          previewParam === "ring" ||
          previewParam === "earring" ||
          previewParam === "bracelet" ||
          previewParam === "necklace"
        )
          enabledCategories.push("jewelry_random");
        if (
          previewParam === "coastal" ||
          previewParam === "megafauna" ||
          previewParam === "oceanic"
        )
    ...
    if (markerParam) {
      const goTo = MapBase.markers.find(
        (_m) => _m.text === markerParam && _m.isCurrent,
      );
      if (!goTo) return;
      ...
      if (!enabledCategories.includes(goTo.category)) {
        ...
        MapBase.addMarkers();
        document
          .querySelector(`[data-type="${goTo.category}"]`)
  ...
  resetMarkersDaily: function () {
    ...
    if (
      localStorage.getItem("rdr2collector.date") == null ||
      date != localStorage.getItem("rdr2collector.date")
    ) {
      MapBase.markers.forEach((marker) => {
        ...
        if (
          InventorySettings.isEnabled &&
          !marker.isRandomizedItem &&
  ...
  initFuse: function () {
    const dataForSearch = MapBase.markers.map((marker) => ({
      ...
      itemTranslationKey: Language.get(marker.itemTranslationKey)
        .toLowerCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, ""),
    }));
    ...
    this.fuseOnSearch = new Fuse(dataForSearch, {
      keys: ["itemTranslationKey"],
      threshold: 0.4,
    });
    this.fuseOnQuerySuggestions = new Fuse(dataForQuerySuggestions, {
      keys: ["name"],
      threshold: 0.6,
    });
  },
        ...
        uniqueSearchMarkers = MapBase.markers.filter((marker) =>
          searchTerms.some(
            (term) =>
              ...
              Language.get(marker.itemTranslationKey)
                .toLowerCase()
                .normalize("NFD")
                .replace(/[\u0300-\u036f]/g, "")
                .includes(term),
  ...
  onQuerySuggestions: function (searchString) {
    ...
    suggestionsContainer.querySelectorAll(".query-suggestion").forEach((el) => {
      el.addEventListener("click", (e) => {
        ...
        const baseStr =
          splitStr.length > 1 ? splitStr.slice(0, -1).join(";") + "; " : "";
  ...
  addMarkers: function (refreshMenu = false) {
    ...
    if (RouteSettings.generateOnVisit) Routes.generatePath(true);
  },

  removeItemFromMap: function (
    day,
    text,
    subdata,
    category,
    ...
    const markers = MapBase.markers.filter(function (marker) {
      return (
        marker.day == day && (marker.text == text || marker.subdata == subdata)
      );
    });
    ...
    const subdataCategoryIsDisabled =
      text == subdata &&
      !document
        .querySelector(`[data-type="${subdata}"] .collectible-text p`)
    ...
    markers.forEach((marker) => {
      ...
      if (marker.isCurrent) {
        if (
          (marker.subdata == subdata && subdataCategoryIsDisabled) ||
          marker.canCollect
        ) {
      ...
      marker.item &&
        marker.item.changeAmountWithSideEffects(
          skipInventory ? 0 : changeAmount,
        );

      if (!InventorySettings.isEnabled) {
        const markerEl = document.querySelector(
          `[data-type="${marker.legacyItemId}"] .collectible-text p`,
        );
    ...
    if (RouteSettings.ignoreCollected) Routes.generatePath();

    Menu.refreshItemsCounter();
  },

  addMarkerOnMap: function (marker, ignoreToolSetting) {
    if (!(marker.isVisible && (ignoreToolSetting || marker.toolAccepted())))
      return;

    marker.recreateLMarker();

    Layers.itemMarkersLayer.addLayer(marker.lMarker);
    if (Settings.isMarkerClusterEnabled) Layers.oms.addMarker(marker.lMarker);
  },
  ...
  addFastTravelMarker: function () {
    ...
    if (enabledCategories.includes("fast_travel")) {
      Object.entries(MapBase.fastTravelData).forEach(([key, value]) => {
        ...
        marker.bindPopup(
          `<h1>${Language.get(value.text + ".name")}</h1><p></p>`,
        );
  ...
  loadLootTable: function () {
    return Loader.promises["loot"].consumeJson((data) => {
      ...
      console.info(
        "%c[Loot Tables] Loaded!",
        "color: #bada55; background: #242424",
      );
    });
  },

  debugMarker: function (
    lat,
    long,
    name = "Debug Marker",
  ...
  addCoordsOnMap: function (coords) {
    ...
    if (Settings.isCoordsOnClickEnabled) {
      ...
      ["click", "touchend"].forEach((event) => {
        document
          .getElementById("lat-lng-container-start")
          .addEventListener(event, () => Routes.setCustomRouteStart(lat, lng), {
            once: true,
          });
        document
          .getElementById("lat-lng-container-generate")
          .addEventListener(
            event,
            () => Routes.setCustomRouteStart(lat, lng, true),
            { once: true },
          );
        document
          .getElementById("lat-lng-container-close-button")
          .addEventListener(
            event,
            () => {
  ...
  updateTippy: function (loc = "") {
    if (Settings.isDebugEnabled) console.log("UpdateTippy called from", loc);

    // This is here to deal with stacked onMap updates (show all/hide all)
    return new Promise((resolve) => setTimeout(resolve, 300)).then(() => {
      if (Settings.isDebugEnabled) console.log("Updating MapBase Tippy...");
