const Language = {
  ...
  init: function () {
    ...
    const fetchTranslations = langs.map(async (language) => {
      ...
      if (!response.ok) {
        throw new Error(
          `${response.status} ${response.statusText} on ${response.url}. Failed to fetch translation data for ${language}`,
        );
      }
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
          if (xhr.status === 200) {
            ...
            console.error(
              `Failed to fetch translation data for ${Settings.language}. Status code: ${xhr.status}`,
            );
          }
        }
      };
  ...
  _postTranslation: function () {
    const diffDays = Math.floor(
      (new Date() - new Date("November 26, 2018")) / (1000 * 60 * 60 * 24),
    );
    document.querySelector("#sub-header .cycle-date").setAttribute(
      "data-tippy-content",
      Language.get("menu.span_game_released")
    ...
    document
      ...
      .setAttribute("placeholder", Language.get("menu.search_placeholder"));
    ...
    document
      ...
      .setAttribute("title", Language.get("menu.back_to_top"));

    document
      .querySelectorAll("code-box")
      .forEach(
        (box) =>
          (box.shadowRoot.querySelector(".copied-toast").textContent =
            Language.get("map.link_copied")),
      );

    document
      .querySelectorAll(".pcr-reset")
      .forEach((btn) => (btn.value = Language.get("map.color_picker.reset")));
    document
      .querySelectorAll(".pcr-save")
      .forEach((btn) => (btn.value = Language.get("map.color_picker.save")));
