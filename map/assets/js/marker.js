class Marker {
  ...
  constructor(preliminaryMarker, cycleName, category) {
    ...
    this.subdata = ["egg", "flower"].includes(this.category)
      ? this.itemId.replace(`${this.category}_`, "")
      : undefined;
    this.legacyItemId = this.subdata || this.text;
    this.item =
      this.category === "random"
        ? undefined
        : Item.items.find((item) => item.itemId === this.itemId);
    ...
    this.lootTable = (() => {
      switch (this.category) {
        case "fossils_random": {
          if (this.text.includes("_mud_")) return "fossils_buried_mud";
          else if (this.text.includes("_snow_dirt_"))
            return "fossils_buried_dirt_snow";
          else if (this.text.includes("_snow_")) return "fossils_buried_snow";
          else if (this.text.includes("_water_")) return "fossils_buried_water";
        }
        case "arrowhead": {
          if (this.tool === 1) {
  ...
  get canCollect() {
    if (this.isCollected) {
      ...
      const stackName =
        this.category === "flower" ? "flowersSoftStackSize" : "stackSize";
  ...
  toolAccepted() {
    return Settings.toolType >= this.tool || Settings.toolType === -this.tool
      ? true
      : false;
  }
  ...
  colorUrls() {
    ...
    const markerColor = MapBase.isPreviewMode
      ? "by_cycle"
    ...
    if (this.item && this.item.isWeekly() && Settings.showWeeklySettings) {
      ...
      const settingsColor = JSON.parse(
      ...
      base =
        ["blue", "orange", "pink", "darkpurple", "darkred", "darkblue"][
          +this.cycleName - 1
        ] || "lightred";
    } else {
      base = markerColor;
  ...
  popupContent() {
    ...
    snippet
      .querySelector(".cycle-display")
      .classList.toggle("hidden", !Settings.isCyclesVisible);

    snippet
      .querySelector(".marker-troubleshooting")
      .addEventListener("click", () => markerTRBLModal.show());

    const importanceBtn = snippet
      .querySelector(".marker-popup-links")
      .querySelector(
        '[data-text="map.mark_important"], [data-text="map.unmark_important"]',
      );
    importanceBtn.addEventListener("click", (e) => {
      e.preventDefault();
      this.item.isImportant = !this.item.isImportant;
      const currTarget = e.currentTarget;
      currTarget.setAttribute(
        "data-text",
        this.item.isImportant ? "map.unmark_important" : "map.mark_important",
      );
      currTarget.textContent = this.item.isImportant
        ? Language.get("map.unmark_important")
        : Language.get("map.mark_important");
    });
    importanceBtn.parentElement.style.display = !this.isRandomizedItem
      ? ""
      : "none";
    ...
    if (!Cycles.isSameAsYesterday(this.category) && !unknownCycle) {
      ...
      if (unknownCycle) {
        snippet.querySelector(
          '[data-text="map.same_cycle_yesterday"]',
        ).style.display = "none";
      } else {
        snippet.querySelector(
          '[data-text="map.unknown_cycle_description"]',
        ).style.display = "none";
      }
    }
    snippet.querySelector('[data-text="weekly.desc"]').style.display =
      this.item && this.item.isWeekly() ? "" : "none";
    snippet.querySelector('[data-text="map.item.unable"]').style.display = this
      .buggy
      ? ""
      : "none";
    ...
    if (!this.buggy && this.tool === 0) {
      ...
      const imgName = this.buggy
        ? "cross"
        : { 1: "shovel", 2: "magnet" }[this.tool];
      toolImg.src = `assets/images/${imgName}.png`;
    }
    if (!Settings.isDebugEnabled)
      snippet.querySelector(".popupContentDebug").style.display = "none";
    if (!this.isRandomizedItem)
      snippet.querySelector(
        '[data-text="map.view_loot"]',
    ...
    if (
      ...
      InventorySettings.isPopupsEnabled &&
      this.category !== "random" &&
      this.item
    ) {
      ...
      small.classList.toggle(
        "text-danger",
        this.item.amount >= InventorySettings.stackSize,
      );
  ...
  updateColor() {
    ...
    lMarkerEl
      ?.querySelector("img.marker-contour")
      .setAttribute("src", contourUrl);
    lMarkerEl?.querySelector("img.background")?.setAttribute("src", bgUrl);
  }

  updateOpacity(
    opacity = Settings.markerOpacity,
    isInvisibleRemovedMarkers = Settings.isInvisibleRemovedMarkers,
  ) {
    let targetOpacity;
    if (this.canCollect) {
  ...
  recreateLMarker(
    isShadowsEnabled = Settings.isShadowsEnabled,
    ...
    const match = bgUrl.match(
      /marker_#(?<hex>[0-9a-fA-F]{6})(?<alpha>[0-9a-fA-F]{2})?\.png$/,
    );
    const hexColor = match
      ? `#${match.groups.hex}${match.groups.alpha || ""}`
      : null;
    ...
    if (Settings.isPopupsEnabled) {
      this.lMarker.bindPopup(this.popupContent.bind(this), {
        minWidth: 300,
        maxWidth: 400,
      });
    }
  ...
  static createMarkerColorCustomPanel() {
    ...
    const markerColors = Object.values(colorNameMap).sort((...args) => {
      const [a, b] = args.map((color) =>
        Language.get(`map.user_pins.color.${color}`),
      );
    ...
    const defaultColors = {
      ...
      heirlooms: "pink",
      random: "lightgray",
      random_spot_metal_detector_chest: "lightgray",
      random_spot_shovel: "lightgray",
      random_spot_metal_detector_shallow: "gray",
    };
    const baseColors = structuredClone(defaultColors);

    const randomCategories = [
      "random_spot_metal_detector_chest",
      "random_spot_metal_detector_shallow",
      "random_spot_shovel",
    ]; // divide random spots to metal detector chest & shallow & shovel
    const itemCollections = Collection.collections;
    const possibleCategories = [
      ...new Set(MapBase.markers.map(({ category }) => category)),
    ]
      // fossils categories => fossils_random, random => random_spot_metal & random_spot_shovel
      .filter(
        (category) =>
          !["coastal", "oceanic", "megafauna", "random"].includes(category),
      );

    const categories = [...possibleCategories, ...randomCategories].sort(
      (...args) => {
        const [a, b] = args.map((element) => {
          const index = itemCollections
    ...
    document
      .getElementById("custom-marker-color-modal")
      .querySelector("#custom-colors")
      .appendChild(translatedContent);

    wrapper
      .querySelectorAll(".pickr-custom-marker-color")
      .forEach((inputEl) => {
        ...
        const pickr = Pickr.create({
          ...
          localStorage.setItem(
            "rdr2collector.customMarkersColors",
        ...
        const btnSave = document
          .getElementById(`${cat}-custom-color`)
          .querySelector(".pcr-save");
        ...
        btnReset.addEventListener("click", () =>
          pickr.setColor(colorNameToHexMap[defaultColors[cat]]),
        );
      });
  }
  ...
  static init() {
    [
      ...
      document
        ...
        .querySelectorAll(`option[data-text$="${proxy[settingName]}"]`)
        .forEach((option) => {
      ...
      document.querySelector(domSelector).addEventListener("change", (e) => {
        ...
        proxy[settingName] = selectedOption
      ...
      Object.entries(data).forEach(([category, allCycles]) => {
        Object.entries(allCycles).forEach(([cycleName, markers]) => {
          markers.forEach((preliminaryMarker) => {
            ...
            if (
              !["fossils_random", "jewelry_random", "random"].includes(
                marker.category,
              )
            ) {
